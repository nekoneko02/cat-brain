...
class DQNAgent:
    def __init__(self, agent_name, env, buffer_size=10000, batch_size=64, learning_rate=1e-4, gamma=0.995, epsilon=1.0, epsilon_min=0.01, epsilon_decay=0.995, device=None):
    ...
    def set_model(self, model, target_model=None):
        self.model = model.to(self.device)
        if target_model is None:
            self.target_model = type(model)(self.state_shape, self.action_space.n).to(self.device)
    ...
    def act(self, state):
        ...
        state = torch.FloatTensor(state).unsqueeze(0).to(self.device)  # バッチ次元を追加
    ...
    def replay(self):
        ...
        # サンプルからデータを取得
        states = sample.data["state"].to(self.device)
        actions = sample.data["action"].squeeze(-1).to(self.device)
        rewards = sample.data["reward"].squeeze(-1).to(self.device)
        next_states = sample.data["next_state"].to(self.device)
        dones = sample.data["done"].squeeze(-1).to(self.device)
